...
#Plots given diffusion data of Zn and XeH2, data obtained from other experiment
#Used to compare the computed values againsted these found values
def plot_example_diffusion_data():
    T = np.linspace(10,1100,100)
    PD_out = 1.31E-4 * T**1.91
    PD_Zn = 4.33E-5 * T**2.041

    PD_xeh2 = ((3.68E-5 * T**1.712)/np.exp(16.9/T)) * (101325/1E4)

    MD_T = np.array([300,400,500,600,700,800,900,1000])
    MD_PD = 1.31E-4 * MD_T**1.91

    plt.plot(T,PD_out,label="MD Function Sn",alpha=0.4,color='yellow')
    plt.plot(T,PD_Zn,label="Zinc Data",alpha=0.4,color='orange')
    plt.plot(T,PD_xeh2,label = "Xe H2 Data",alpha=0.4,color='red')
...
if __name__ == '__main__':
    ...
    #Plots our found computed data as well as the data given from plot_example_diffusion_data
    plt.figure(dpi=200)
    plot_example_diffusion_data()
